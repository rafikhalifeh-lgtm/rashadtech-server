...
async function respondAdminLoginSuccess(res, adminEmail) {
  const token = createSession('admin', adminEmail);
  try {
    const data = await readJsonBinRaw({ fast: true });
...
app.post('/auth/admin-login', async (req, res) => {
  ...
  if (!code) {
    recordAdminLoginFailure(ip);
    return res.status(401).json({ error: 'Enter the 6-digit code from your authenticator app.' });
  }
  ...
  return respondAdminLoginSuccess(res, 'admin');
});
